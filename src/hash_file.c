...
HT_ErrorCode HT_CreateIndex(const char *filename, int buckets) {//create an empty file,HT_init first
  ...
  for (i=0; i<buckets; i++){
      if ( size + 4 < BF_BLOCK_SIZE ) {
          memcpy(data+offset, "-1", 4); // 4 digits,offset = 4 to find next bucket
...
HT_ErrorCode HT_PrintAllEntries(int indexDesc, int *id) {
  ...
  if ( id != NULL ) {
      ...
      int blockBuckets = BF_BLOCK_SIZE / 4; //how many buckets a block can store
      ...
      while(bucket_num != 1){
          ...
          if ( atoi(str) != -1){
            ...
            CALL_BF(BF_GetBlock(fd, atoi(str), new_blockB));
